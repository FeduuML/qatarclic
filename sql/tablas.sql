...
CREATE TABLE cooldown_password (
    id int NOT NULL AUTO_INCREMENT,
    user_id int NOT NULL,
    cooldown timestamp NULL,
    PRIMARY KEY(id),
    FOREIGN KEY (`user_id`) REFERENCES `users`(`id`)
);

CREATE TABLE cooldown_username (
    id int NOT NULL AUTO_INCREMENT,
    user_id int NOT NULL,
    cooldown timestamp NULL,
    PRIMARY KEY(id),
...
CREATE TABLE seleccion (
    ...
    PRIMARY KEY(id),
    FOREIGN KEY (`user_id`) REFERENCES `users`(`id`),
    FOREIGN KEY (`seleccion_id`) REFERENCES `teams`(`id`)
);

INSERT INTO `seleccion` VALUES('1', '1', '9');
SELECT u.id FROM users u WHERE u.id = 1;
SELECT t.id FROM teams t WHERE t.id = 9;
...
INSERT INTO `news` VALUES('4', 'Lorrrran', 'Oficial: Giovani Lo Celso se pierde el mundial', 'Una de las figuras del seleccionado argentino deberá ser operado de la lesión muscular que sufrió en la parte posterior de la pierna derecha: la recuperación será de entre seis y ocho semanas, por lo que no podrá estar en la cita máxima del fútbol mundial.', '972080.jpg', '2022-11-12 16:57:57');
INSERT INTO `news` VALUES('5', 'Lorrrran', 'Scaloni confirmó los 26 jugadores', 'Emiliano Martínez, Franco Armani, Gerónimo Rulli, Gonzalo Montiel, Nahuel Molina, Cristian Romero, Germán Pezzella, Nicolás Otamendi, Lisandro Martínez, Marcos Acuña, Nicolás Tagliafico, Juan Foyth, Leandro Paredes, Guido Rodríguez, Rodrigo De Paul, Exequiel Palacios, Alexis Mac Allister, Alejandro Gómez, Enzo Fernández, Lionel Messi, Lautaro Martínez, Julián Álvarez, Ángel Di María, Nicolás González, Paulo Dybala y Joaquín Correa.', '985663.jpg', '2022-11-12 17:03:42');
